...
- 公开 JSON：`GET /api/v1/sh000001/latest.json`，成功响应使用 `{ "code": 0, "data": { "msg": "success", "items": [...] }, "status": 200 }` 结构；`items` 中每条记录仅返回 `signal_date`、`predicted_next_day_return`、`predicted_direction`、`predicted_next_day_close`、`confidence`、`actual_next_day_return`、`direction_prediction_correct`，其中 `predicted_direction` 为 `up` 或 `down`。最多 60 个已结算交易日＋下一交易日；旧 `/api/v1/sh000001/latest.csv` 地址兼容返回相同 JSON。
