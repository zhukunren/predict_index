...
- 公开 JSON：`GET /api/v1/sh000001/latest.json`，最多 60 个已结算交易日＋下一交易日，仅返回 `signal_date`、`predicted_next_day_return`、`predicted_direction`、`predicted_next_day_close`、`confidence`、`actual_next_day_return`、`direction_prediction_correct`；旧 `/api/v1/sh000001/latest.csv` 地址兼容返回相同 JSON。
