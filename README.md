...
当前服务采用 **期权＋资金流组合** 作为正式生产模型，原生产基线、资金流信号、资金流＋价格特征组合作为三个影子模型。四个模型共享行情、独立保存预测，每日上海时间 **20:15** 刷新。管理员在 `/admin/models` 查看同日期统计、逐日预测、方向分歧和事前表现，并可自定义统计交易日数、导出对比 CSV。
...
管理员密码只在首次创建本地管理员账号时读取，数据库中仅保存 Argon2 哈希。公开 CSV 直接读取已发布的不可变文件，支持 `GET`、`HEAD`、`ETag / If-None-Match` 和 `304`，访问时不会运行预测或请求行情。归档完整性校验失败时返回 `503`。安装新代码不会中断旧版本 CSV 的下载；新发布仍须通过历史账本的一致性检查。
